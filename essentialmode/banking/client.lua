    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('es_rp:notify', "Drücke INPUT_CONTEXT (~r~Standart E~w~) um die Bank zu nutzen")
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Das geht nicht vom Auto aus!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Spieler ist nicht Online!");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Spieler nicht nah genug!");
  end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Spieler nicht Online!");
  end
end)
