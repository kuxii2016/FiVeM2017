...
local heliports = {
	{name="Heli Hangar", colour=2, id=64, x=-725.428771972656, y=-1443.3955078125, z=5.00051927566528, axe = 120.000},
}
...
function MenuHeli()
    ...
    MenuTitle = "Hangar"
    ClearMenu()
    Menu.addButton("Parke Heli","RentrerHelico",nil)
    Menu.addButton("Hole Heli","ListeHelico",nil)
...
function OptionHeli(heliID)
    ...
    MenuTitle = "Options"
    ClearMenu()
    Menu.addButton("Holen", "Spawn", heliID)
    Menu.addButton("Zurück", "Helies", nil)
end

function Spawn(heliID)
	local heliID = heliID
...
Citizen.CreateThread(function()
	...
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString('Heli Verkauf')
	...
	while true do
		...
		if GetDistanceBetweenCoords(vente_location[1],vente_location[2],vente_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			drawTxt('Drücke ~g~E~s~ um einen Hubschrauber zu Verkaufen',0,1,0.5,0.8,0.6,255,255,255,255)
			if IsControlJustPressed(1, 86) then
	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(caissei) then	
			if plate ~= platecaissei then					
				drawNotification("Parke Hubschrauber")
			else
				Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(caissei))
				drawNotification("Heli geparkt")
...
AddEventHandler('helishop:SelHeli', function(heli, plate)
	...
	Citizen.CreateThread(function()		
		...
		if DoesEntityExist(caissei) then
			if plate ~= platecaissei then
				drawNotification("Lade Hubschrauber")
			else
				Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(caissei))
