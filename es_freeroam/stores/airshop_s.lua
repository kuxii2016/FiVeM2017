...
AddEventHandler('CheckMoneyForVeh', function(vehicle, price)
    ...
    print(player)
      ...
      MySQL:executeQuery("UPDATE users SET personalair='@vehicle' WHERE identifier = '@username'",
      {['@username'] = player, ['@vehicle'] = vehicle})
      -- Trigger some client stuff
      TriggerClientEvent('FinishMoneyCheckForVeh',source)
      TriggerClientEvent("es_freeroam:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Fahren Sie sicher mit diesem neuen Auto, das ist nicht Carmageddon!\n")
    else
      -- Inform the player that he needs more money
    TriggerClientEvent("es_freeroam:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Leider nicht genug Geld!\n")
	end
end)
end)
...
TriggerEvent('es:addCommand', 'pv', function(source, user)
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    local executed_query = MySQL:executeQuery("SELECT * FROM users WHERE identifier = '@username'",{['@username'] = player})
  	local result = MySQL:getResults(executed_query, {'personalair'})

    if(result)then
		for k,v in ipairs(result)do
      print(v.personalair)
      vehicle = v.personalair
    end
  end
