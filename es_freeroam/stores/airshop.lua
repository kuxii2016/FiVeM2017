local vehshop = {
	...
	menu = {
		...
		["helies"] = {
			...
			name = "helies",
			buttons = {
				{name = "unbewafnet", description = ''},
				{name = "leichtbewaffnet", description = ''},
				{name = "schwerbewaffnet", description = ''},
		...
		["unbewafnet"] = {
			...
			name = "unbewafnet",
			buttons = {
				{name = "Cargobob", costs = 2200000, description = {}, model = "cargobob"},
				{name = "Maverick", costs = 780000, description = {}, model = "maverick"},
				{name = "SuperVolito", costs = 2113000, description = {}, model = "supervolito"},
				{name = "Swift", costs = 1500000, description = {}, model = "swift"},
			}
		},
		["leichtbewaffnet"] = {
			title = "leichtbewaffnet",
			name = "leichtbewaffnet",
			buttons = {
				{name = "Valkyrie", costs = 3790500 , description = {}, model = "valkyrie"},
				{name = "Annihilator", costs = 1825000, description = {}, model = "annihilator"},
			}
		},
		["schwerbewaffnet"] = {
			title = "schwerbewaffnet",
			name = "schwerbewaffnet",
			buttons = {
				{name = "Savage", costs = 2593500, description = {}, model = "savage"},
			}
		},
...
{entering = {1733.9177246094,3296.8081054688,41.223514556885}, inside = {1728.7237548828,3312.458984375,41.223514556885, 120.1953}, outside = {1733.9177246094,3296.8081054688,41.223514556885}},
}
...
function ShowVehshopBlips(bool)
	if bool and #vehshop_blips == 0 then
		for station,pos in pairs(vehshop_locations) do
			...
			AddTextComponentString('Air shop')
...
function CloseCreator()
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			personalair = CreateVehicle(model,pos[1],pos[2],pos[3],pos[4],true,false)
			SetModelAsNoLongerNeeded(model)
			for i,mod in pairs(mods) do
				SetVehicleModKit(personalair,0)
				SetVehicleMod(personalair,i,mod)
			end
			SetVehicleOnGroundProperly(personalair)
			SetVehicleHasBeenOwnedByPlayer(personalair,true)
			local id = NetworkGetNetworkIdFromEntity(personalair)
			SetNetworkIdCanMigrate(id, true)
			Citizen.InvokeNative(0x629BFA74418D6239,Citizen.PointerValueIntInitialized(personalair))
			SetVehicleColours(personalair,colors[1],colors[2])
			SetVehicleExtraColours(personalair,extra_colors[1],extra_colors[2])
			--local blip = AddBlipForEntity(personalair)
			--SetBlipSprite(blip,326)
			--BeginTextCommandSetBlipName("STRING")
			--AddTextComponentString('Personal vehicle')
			--EndTextCommandSetBlipName(blip)
			--personalair_blip = blip
			TaskWarpPedIntoVehicle(GetPlayerPed(-1),personalair,-1)
			SetEntityVisible(ped,true)
			...
			for i,button in pairs(menu.buttons) do
				if i >= vehshop.menu.from and i <= vehshop.menu.to then
					...
					if button.costs ~= nil then
						if vehshop.currentmenu == "unbewafnet" or vehshop.currentmenu == "leichtbewaffnet" or vehshop.currentmenu == "schwerbewaffnet" --or vehshop.currentmenu == "motorcycles" 
						then
